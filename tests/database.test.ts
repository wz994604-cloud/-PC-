...
describe("draw repository", () => {
  it("stores draws once and queries newest first", async () => {
    expect(await saveDraws([draw("2", [4, 5, 6]), draw("1", [1, 2, 3])])).toBe(2);
    expect(await saveDraws([draw("2", [4, 5, 6])])).toBe(0);
    expect((await getDrawHistory()).map(({ issue }) => issue)).toEqual(["2", "1"]);
  });

  it("respects history limits", async () => {
    await saveDraws([draw("2", [4, 5, 6]), draw("1", [1, 2, 3])]);
    expect(await getDrawHistory(1)).toHaveLength(1);
  });
});
