...
import { saveShadowPredictionOnce } from "@/lib/db/shadow-prediction-repository";
...
describe("public prediction APIs are read-only",()=>{
  ...
  it("reads the latest immutable prediction without calling upstream",async()=>{
    const prediction=createV02ShadowPrediction([draw])!;
    await saveShadowPredictionOnce(prediction,"2026-01-01T00:00:00Z");
    const fetchMock=vi.fn();vi.stubGlobal("fetch",fetchMock);
    const response=await getPrediction(),body=await response.json();
    expect(body.data).toMatchObject({issue:"101",modelVersion:"v0.2-candidate-b"});
